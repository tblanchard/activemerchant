  ...
  module Billing #:nodoc:
    ...
    class TranCloudGateway < Gateway
      ...
      def purchase(money, creditcard, options = {})
        ...
        logger.warn 'REQUEST: ' + JSON.pretty_generate(transRequest)
        ...
        logger.warn 'RESPONSE: ' + JSON.pretty_generate(response)
      ...
      def reset
        ...
        trs = transRequest[:TStream][:Transaction]
        logger.warn 'REQUEST: ' + transRequest.to_json
        response = RestClient::Request.execute(
          :method => :post, 
          :url => url, 
          :headers => headers, 
          :payload => transRequest.to_json, 
          :timeout => 120) {|response, request, result| JSON.parse(response, :symbolize_names => true) }
        logger.warn 'RESPONSE: ' + response.to_json
